...
def save_models(query, cat, link, scrape_data, reschedule, username):
    ...
    link = Link.objects.get_or_create(
        keyword=keyword, category=category, link=link, scrape_data=scrape_data
    )[0]
    link.save()

    profile_update, created = CrawledLinks.objects.get_or_create(
        userprofile=userprofile, link=link, reschedule=reschedule
    )
    profile_update.save()
